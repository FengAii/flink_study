...
class HainiuSource extends SourceFunction[String] {
  var num = 0
  var isCancel = true
...
object SourceFunctionWordCount {
  def main(args: Array[String]): Unit = {
    ...
    // 使用自定义的source
    val text = env.addSource(new HainiuSource)



    // 定义operators，作用是解析数据, 分组, 窗口化, 并且聚合求SUM
    val windowCounts = text.flatMap(new FlatMapFunction[String,(String,Int)] {
